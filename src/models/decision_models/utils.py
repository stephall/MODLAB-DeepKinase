# models/decision_models/utils.py

# Import custom modules
from . import model_definitions

# Define the function used to define models
def define_model(config_dict, x_combined_dim):
    ...
    try:
        ...
        err_msg = f"The decision model with name '{config_dict['name']}' is not defined in the file 'decision_model/model_definitions.py'."
        raise AttributeError(err_msg)

    # Instantiate the model object with the config dictionary and the dimensions of the latent spaces
    return model_obj(config_dict, x_combined_dim)